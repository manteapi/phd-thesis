\chapter[Adaptive physics-based animation]{Adaptive techniques and models for physics-based animation}
...
The adaptation is designed to meet a given criteria which depends on the application. 
Examples of frequently used criteria include reducing the overall computational complexity without loss of quality, improving the quality of real-time simulation, or simulating more precisely the parts of the scene which the user is currently interacting with.
\paragraph*{}
...
For instance, a system entering a highly nonlinear regime, such as during fracture, requires smaller time steps to maintain the desired degree of accuracy.
...
For methods that use implicit integration or other globally coupled schemes, such as grid-based fluids with a global pressure solve, this is the only possible approach.
...
This approach has the same computational advantage because evaluation of $f$ is the most expensive part in explicit methods.
...
This approach is implemented as an event-driven simulation loop, using a priority queue to schedule the updates for all the forces in order.
...
\begin{figure}[!h]
	...
	\begin{subfigure}[b]{0.35\linewidth}
		\centering
		\includegraphics[width=\linewidth]{images/starAdaptivity-cgf2016/freezing-graph.png}
...
Ando et al.~\cite{Ando2013}~use a flexible sizing function that combines multiple criteria to set the desired resolution. Among them the depth of the liquid, the camera viewpoint, the fluid surface curvature and the norm of the strain rate, $e = \|\nabla\mathbf u\|_F$, in order to preserve detailed motions.
...
In such methods, one maintains a high-resolution representation of the surface as a triangle mesh that is updated at each time step.
...
\section{Miscellaneous techniques for spatial adaptivity} \label{sec pr adaptivity}
...
Thus, instead of having a complexity dependent on the simulation mesh resolution, it only depends on the size of the reduced basis (the number of deformation modes) which is much smaller.
...
Therefore model reduction, \emph{alone}, is truly efficient for smooth global deformations and predictable scenarios where it is possible to build a basis that remains constant over time. It is also only useful in situations where a limited number of modes can adequately describe the system.
...
Highly nonlinear folds and wrinkles, which are very hard problems for subspace simulation, can then be reproduced. Dynamics is damped near tightly constrained regions such as sleeves, but this may be acceptable in practice for animating tight clothing.
...
A more versatile approach is to use multiple independently moving grids, centered at each moving object in the scene.
